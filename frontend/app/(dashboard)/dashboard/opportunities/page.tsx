...
/**
 * Automation Opportunities Page (Feasible INR Standards & Tooltip Architecture)
 */
import { useEffect, useState } from 'react';
import { motion } from 'framer-motion';
import { Lightbulb, Clock, IndianRupee, TrendingUp, Zap, ChevronDown, ChevronUp, Filter, Loader2, Sparkles } from 'lucide-react';
...
const OPP_TOOLTIP = {
  ...
  techStack: ['TypeScript 5', 'Quart REST APIs', 'OpenAI GPT-4o', 'TailwindCSS v3', 'Framer Motion'],
  implementation: 'Evaluates process throughput, manual effort, and team headcount against a 100-point composite scoring matrix (Feasibility + Impact + Financial Savings).',
  howItWorks: 'Parses repetitive operational workflows into ranked vectors so leadership can prioritize quick-win automation deployments.',
};
...
function OpportunityCard({ opp, index }: { opp: AutomationOpportunity; index: number }) {
  ...
  return (
    <motion.div
      ...
      className="p-8 rounded-3xl border border-white/10 bg-[#0e1122] hover:bg-white/[0.03] transition-all space-y-6 shadow-lg"
    >
      <div className="flex flex-col md:flex-row md:items-start justify-between gap-6">
        <div className="flex-1 space-y-3">
          ...
          <p className="text-sm text-slate-300 leading-relaxed max-w-3xl">{opp.description}</p>
        </div>

        {/* Scores */}
        <div className="flex items-center gap-6 p-4 rounded-2xl border border-white/10 bg-white/[0.03] flex-shrink-0">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="p-4 rounded-2xl border border-white/10 bg-white/[0.03]">
          <div className="flex items-center gap-2 text-slate-400 text-xs font-medium mb-1">
            <IndianRupee className="w-3.5 h-3.5" /> Estimated Annual Savings
          </div>
      ...
      <div className="pt-2">
        ...
        {expanded && (
          ...
          >
            <p className="text-xs font-medium text-slate-400 uppercase tracking-wider">Recommended Enterprise Tools</p>
    ...
    <div className="space-y-8 max-w-7xl mx-auto pb-12">
      ...
      >
        <div className="space-y-1">
          <div className="flex items-center gap-3">
            ...
            <FeatureInfoTooltip info={OPP_TOOLTIP} />
          </div>
        ...
        <div className="flex items-center gap-3">
          <div className="px-5 py-3 rounded-2xl border border-emerald-500/20 bg-emerald-500/10 text-emerald-300">
            <p className="text-xs text-slate-400">Total Savings Identified</p>
          ...
          <div className="px-5 py-3 rounded-2xl border border-indigo-500/20 bg-indigo-500/10 text-indigo-300">
            <p className="text-xs text-slate-400">Weekly Hours Recovered</p>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Rank Vectors By:</span>
